...
export default function AuthControls({ isAuthenticated = false, onLoginClick, onSignupClick, onLogoutClick }: Props) {
    if (!isAuthenticated) {
        return (
            <div className="flex gap-3">
                <button className="bg-green-500 rounded-full px-4 py-3 text-white text-xl
                hover:bg-green-700 hover:scale-105 transform transition duration-300">
                    Log In
                </button>

                <button className="border-2 border-green-500 rounded-full px-4 py-3 text-green-500 text-xl
    ...
    return (
        <button className="bg-red-500 rounded-full px-4 py-3 text-white text-xl
